...
c_rate = 1.2 / 10000  # 手续费，手续费>万1.2，私信邢不行（微信：xbx9585)开户，自动交易
t_rate = 1 / 1000  # 印花税


# ===导入数据
df = pd.read_pickle('stock-data-for-choosing.pkl')  # 从pickle文件中读取整理好的所有股票数据
...
# ===构建选股因子
df['因子'] = df['总市值']**3*df['成交额std_5']
#df['因子'] = df['总市值']
#df['因子'] = df['现金流量净额(经营活动)(ttm)'] # bad
#df['因子'] = df['现金流量净额(经营活动)(单季)'] # not good
#df['因子'] = df['现金流量净额(经营活动)_同比']

# ===对股票数据进行筛选
df = df[df['上市至今交易天数'] > 250]  # 删除上市不满一年的股票

#df = df[df['申万一级行业名称'].isin(['汽车'])]  # 对所在行业进行筛选
...
#plt.show()
plt.savefig('choose_stocks_monthly.png',dpi=300)
