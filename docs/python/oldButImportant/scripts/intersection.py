...
idx = np.argwhere(np.diff(np.sign(densityA-densityB)) != 0).reshape(-1) + 0
plt.plot(x_coordinate[idx], densityB[idx], 'ro')
for i in range(len(idx)): 
	plt.text(x_coordinate[i], densityB[i], str(x_coordinate[i]))
#plt.show()
#fig=plt.figure(figsize=(15,10))
#fig.patch.set_color('W')# set the background color
#fig=plt.figure(figsize=(15,10))
